    ...
    <div class="flex flex-wrap -px-2">
        ...
        <div class="w-full md:w-1/2 lg:w-1/3 p-2 ">
            <div
                ...
                <div class="flex flex-col justify-around flex-grow ml-5 text-white">
                    <div class="text-xs whitespace-nowrap">
                        Course Total En Cours
                    </div>
                    <div class="">
                        {{progressCourse.length}}
                    </div>
                </div>
                <div class=" flex items-center flex-none text-white">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5}
...
const totalCourse = ref<Course[]>([])
const progressCourse = ref<Course[]>([])
const finishCourse = ref<Course[]>([])

onMounted(async () => {
try {
    const resp1 = await useAxios().get(`/courses/${user_id}`)
    const resp2 = await useAxios().get(`/courses/progress/${user_id}`)
    const resp3 = await useAxios().get(`/courses/finish/${user_id}`)

    totalCourse.value = resp1.data
    progressCourse.value = resp2.data
    finishCourse.value = resp3.data
} catch (error:any) {
